...
- Current product naming decision: the user-facing tool/form name is **TACATDP Impact Evaluation**.
- Current versioning decision: published form versions generated by TACATDP tooling use UTC `YYYYMMDDHHMMSSmmm`, including seconds and milliseconds, unless an explicit version is supplied.
- Local pyxform compile on 2026-07-12 produced generated version `20260712174458448`, XForm title `TACATDP Impact Evaluation`, and a 22,364,336-byte XML artifact.
- The current Dataverse schema deployer creates multiline text columns with `MaxLength: 1048576`; therefore the compiled full XForm is too large for `FormVersions.XFormXml`.
- Microsoft Dataverse file-column documentation says file columns cannot be set or retrieved like normal columns and require the file APIs/Web API file messages for binary data. It also documents chunked upload/download for large file payloads.
...
- New published XLSForm versions must default to UTC `YYYYMMDDHHMMSSmmm`.
- For example: `20260712174458448`.
- Preserve an explicit supplied version only when the operator intentionally passes one to the compile/seed command.
...
- The import pipeline must:
  ...
  - persist small generated XForm XML directly to `FormVersions.XFormXml`;
  - persist large generated XForm XML through Dataverse-managed file storage, using `FormAttachments.File` or an equivalent form-version file-column path, while keeping `FormVersions` as the assignment/version metadata record;
...
4. **Full XLSForm import**
   ...
   - If XML is larger than `FormVersions.XFormXml`, store the XML as a Dataverse file attachment and update the SPA to load it from that Dataverse-managed file source.
   - Execute Dataverse write only after dry-run output is reviewed and the selected storage path can hold the generated XML.
...
- Generated XForm has form title `TACATDP Impact Evaluation`, form id `tacatdp_impact_evaluation`, UTC millisecond timestamp version, and an instance-name path matching `Customer_ID` and `Customer_Name`.
- Full-form storage uses Dataverse-managed storage. For the current generated artifact, direct `FormVersions.XFormXml` write is blocked because the XML exceeds the deployed multiline text limit.
- Browser edit test:

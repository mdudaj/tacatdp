...
This runbook turns the Phase 3 placeholder scaffolds into an ordered Power Apps Studio workflow. The current deliverable is a maker-ready scaffold pack under `artifacts/`, not manually promoted live `Src` source. Do not publish the app and do not connect production SharePoint/Microsoft Lists until the controlled Windows/Power Apps environment is ready.
...
| Artifact | Purpose |
| --- | --- |
| `app-src/Src/` | Current Power Apps Studio-exported source. Keep this import-safe; do not manually promote generated Phase 3 screens/components into it. |
...
1. `artifacts/powerapps/phase3-scaffolds/` is the source scaffold pack for maker-assisted implementation. Do not overwrite `app-src/Src/` directly from these files.
...
When the placeholder screens are built in Power Apps Studio, seed these collections in Studio and re-export/unpack the app so `app-src/Src/` is generated by supported tooling.
...
- [ ] Import/open succeeds before any generated scaffold is promoted into live source.
...
1. Components are created in Power Apps Studio or remain queued in `artifacts/powerapps/phase3-scaffolds/components/`.
2. Screen skeletons are created in Power Apps Studio or remain queued in `artifacts/powerapps/phase3-scaffolds/screens/`.
3. Placeholder data state is visible and isolated.
4. The app package can be imported/opened from the Studio-exported source baseline; generated scaffolds are not treated as live source until Studio or supported tooling normalizes them.

...
Design the TACATDP Power Apps canvas app architecture so the generated Microsoft Lists schema can support an ODK-like guided data-entry experience with skip logic, validation, draft saving, reference data, and clear section progression.
...
4. Each section must save to its target Microsoft Lists section list or child list according to `schemas/xlsform-to-list-mapping.csv`.
...
1. `select_multiple` questions must save selected values as rows in `TACATDP_MultiSelectAnswers`.
2. Production cost stage data must save as rows in `TACATDP_ProductionCostLines`.
...
1. Milestone 1 must use placeholder/local sample data while Microsoft Lists creation is blocked by the controlled Windows environment.
...
2. Every target list from `schemas/xlsform-to-list-mapping.csv` has an app save/read strategy.
...
7. Milestone 1 placeholder screens can be reviewed without live Microsoft Lists.

## Dependencies

- Microsoft Lists must be created and reference data imported.
- Target SharePoint site URL and permissions must be confirmed.
